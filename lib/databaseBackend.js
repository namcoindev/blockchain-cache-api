  ...
  getLastBlockHeader () {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName` FROM `blocks` AS `b` LEFT JOIN `pool_blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` ORDER BY `b`.`height` DESC LIMIT 1', [])
  ...
  getBlockHeaderByHash (blockHash) {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName` FROM `blocks` AS `b` LEFT JOIN `pool_blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` WHERE `b`.`hash` = ? LIMIT 1', [blockHash])
  ...
  getBlockHeaderByHeight (height) {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(block => { topHeight = block.height })
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName` FROM `blocks` AS `b` LEFT JOIN `pool_blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` WHERE `b`.`height` = ? LIMIT 1', [height])
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          return this.query([
            ...
            '`p`.`id` AS `poolId`, `p`.`name` AS `poolName` ',
            'FROM `blocks` AS `b` ',
