...
DatabaseBackend.prototype.legacyGetWalletSyncDataPreflight = function (startHeight, startTimestamp, blockHashCheckpoints) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    /* Get our current top block information */
    try {
      topBlock = await that.getLastBlockHeader()
    } catch (e) {
      return reject(e)
    }
    ...
    var blocks
    try {
      blocks = await that._query('SELECT `hash`, `height`, `timestamp` FROM `blocks` WHERE `height` >= ? AND `height` <= ? ORDER BY `height` ASC LIMIT 1', [topHeight, topBlock.height])
    } catch (e) {
      return reject(e)
    }
...
DatabaseBackend.prototype.legacyGetWalletSyncData = function (startHeight, startTimestamp, blockHashCheckpoints, blockCount, skipCoinbaseTransactions) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    blockQuery.push('WHERE `height` >= ? AND `height` <= ?')
    ...
    var blocks
    try {
      blocks = await that._query(blockQuery.join(' '), [topHeight, topBlock.height, blockCount], true)
    } catch (e) {
      return reject(e)
    }
    ...
    /* Let's see what we got back and then kick it back up to the stack */
    Promise.all(promises).then((dataBlocks) => {
      return resolve({ blocks: dataBlocks, from: topHeight, topBlock: topBlock })
    }).catch((error) => {
      return reject(error)
    })
  })
...
DatabaseBackend.prototype.getTransactionsByBlock = function (blockHash, skipCoinbaseTransactions) {
  skipCoinbaseTransactions = skipCoinbaseTransactions || false
  return new Promise((resolve, reject) => {
    const query = [
      'SELECT `transactions`.*, CAST(`unlockTime` AS CHAR) AS `unlockTimeString` ',
...
DatabaseBackend.prototype.buildLegacyWalletDataBlock = function (block, skipCoinbaseTransactions) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(p).then((cacheResults) => {
      ...
      for (var i = 0; i < obj.transactions.length; i++) {
        /* If we have no inputs and no outputs in our transaction then
           something went terribly/horribly wrong */
        if (obj.transactions[i].inputs.length === 0 && obj.transactions[i].outputs.length === 0) {
          return reject(new Error('Internal data consistency error (NINO) [' + obj.transactions[i].hash + ']'))
        }
...
DatabaseBackend.prototype.getAmountKeys = function (amount, globalIndexes) {
  return new Promise((resolve, reject) => {
    ...
    this._query(query, [amount], true).then((rows) => {
...
DatabaseBackend.prototype._query = function (query, args, nocache) {
  args = args || []
  return new Promise((resolve, reject) => {
