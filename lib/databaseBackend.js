...
Self.prototype.getNodeStats = function () {
  return new Promise((resolve, reject) => {
    ...
    this._query('SELECT * FROM `nodes` ORDER BY `name`', []).then((nodes) => {
...
Self.prototype.getPoolStats = function () {
  return new Promise((resolve, reject) => {
    ...
    this._query('SELECT * FROM `pools` ORDER BY `name`', []).then((pools) => {
      ...
      this._query('SELECT MAX(`timestamp`) AS `timestamp` FROM `pool_polling`', [])
    }).then((rows) => {
      if (rows.length === 0) throw new Error('No timestamp information in the database')
      this._query('SELECT * FROM `pool_polling` WHERE `timestamp` = ?', [rows[0].timestamp || 0])
    }).then((rows) => {
      rows.forEach((row) => {
