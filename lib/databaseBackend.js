...
const Self = function (opts) {
  ...
  this.cacheWalletSyncBlockJSONFor = opts.cacheWalletSyncBlockJSONFor || 60 * 60 * 24 * 14 // 14 days
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(p).then((cacheResults) => {
      ...
      if (cacheResults.length === 1 && cacheResults[0]) {
        ...
        return this.cache.setTtl(util.format('legacy-%s', block.hash), this.cacheWalletSyncBlockJSONFor)
      }
    }).then(() => {
      ...
      if (this.cache) {
        return this.cache.set(cachedBlock.key, obj, this.cacheWalletSyncBlockJSONFor)
      }
    }).then(() => {
      return resolve(obj)
    }).catch((error) => {
