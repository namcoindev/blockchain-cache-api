...
const DatabaseBackend = function (opts) {
  opts = opts || {}
...
DatabaseBackend.prototype._query = function (query, args) {
  args = args || []
...
DatabaseBackend.prototype.checkCache = function (key) {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.setCache = function (key, value, ttl) {
  return new Promise((resolve, reject) => {
    if (!this.cache) return resolve(value)

    this.cache.set(key, value, ttl).then(() => {
      return resolve(value)
    }).catch(() => {
      /* We don't actually error here to prevent the stack
         from not returning a response because the cache is inaccessible */
      return resolve(value)
    })
  })
}
...
DatabaseBackend.prototype.getLastBlockHeader = function () {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query('SELECT * FROM `blocks` ORDER BY `height` DESC LIMIT 1', [])
    }).then((blocks) => {
      if (blocks.length === 0) {
...
DatabaseBackend.prototype.getBlockHeaderByHash = function (blockHash) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, obj, 10)
    }).then(() => {
      return resolve(obj)
    }).catch((error) => {
      return reject(error)
...
DatabaseBackend.prototype.getBlockHeaderByHeight = function (height) {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getRecentChainStats = function () {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getBlockHash = function (height) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, block.hash, 15)
    }).then((hash) => {
      return resolve(hash)
    }).catch((error) => {
      return reject(error)
...
DatabaseBackend.prototype.getBlockHeight = function (hash) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, block.height, 15)
    }).then((height) => {
...
DatabaseBackend.prototype.getBlockCount = function () {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query('SELECT COUNT(*) AS `cnt` FROM `blocks`', [])
    }).then((results) => {
      if (results.length !== 1) {
...
DatabaseBackend.prototype.getTransactionPool = function () {
  return new Promise((resolve, reject) => {
    const cacheName = 'getBlockCount'
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query('SELECT * FROM `transaction_pool`', [])
    }).then((results) => {
      if (!results) return reject(new Error('Could not retrieve current transaction pool'))

      return this.setCache(cacheName, results, 5)
    }).then((rows) => {
      if (rows.length === 0) {
        return resolve([])
      }
...
DatabaseBackend.prototype.getTransactionHashesByPaymentId = function (paymentId) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      if (!results) return reject(new Error('Could not get data'))
...
DatabaseBackend.prototype.getTransaction = function (hash) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, result, 15)
    }).then(() => {
...
DatabaseBackend.prototype.getTransactionInputs = function (hash, trim) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      if (cached) {
        return resolve(checkTrim(cached))
      }

      return this._query('SELECT * FROM `transaction_inputs` WHERE `txnHash` = ? ORDER BY `amount`, `keyImage`', [hash])
    }).then((results) => {
      if (results.length === 0) {
        return resolve([])
      }
      ...
      return resolve(checkTrim(results))
    }).catch((error) => {
      return reject(error)
    })
  })
...
DatabaseBackend.prototype.getTransactionOutputs = function (hash, trim) {
  if (typeof trim === 'undefined') trim = true
  ...
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query('SELECT * FROM `transaction_outputs` WHERE `txnHash` = ? ORDER BY `outputIndex`', [hash])
    }).then((results) => {
      if (results.length === 0) {
        return resolve([])
      }
...
DatabaseBackend.prototype.getBlock = function (hash) {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getTransactionsByBlock = function (blockHash) {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getBlocks = function (height, count) {
  const cnt = count || 30
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, blocks, 5)
    }).then((blocks) => {
      return resolve(blocks)
    }).catch((error) => {
      return reject(error)
...
DatabaseBackend.prototype.getMixableAmounts = function (mixin) {
  mixin = mixin || 3
...
DatabaseBackend.prototype.getInfo = function () {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query('SELECT `payload` FROM `information` WHERE `key` = ?', ['getinfo'])
    }).then((results) => {
      if (results.length === 0) {
...
DatabaseBackend.prototype.getTransactionsStatus = function (hashes) {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getNodes = function () {
  return new Promise((resolve, reject) => {
    const cacheName = 'getNodes'
    ...
    this.checkCache(cacheName)
      ...
      .then(nodes => {
        ...
        return this.setCache(cacheName, tmp, 60)
      })
...
DatabaseBackend.prototype.getNodeStats = function () {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.setCache(cacheName, nodeList, 60)
    }).then((results) => {
      return resolve(results)
    }).catch((err) => {
      return reject(err)
...
DatabaseBackend.prototype.getPools = function () {
  return new Promise((resolve, reject) => {
    const cacheName = 'getPools'
...
DatabaseBackend.prototype.getPoolStats = function () {
  return new Promise((resolve, reject) => {
...
DatabaseBackend.prototype.getAmountKeys = function (amount, globalIndexes) {
  return new Promise((resolve, reject) => {
    ...
    globalIndexes.forEach((num) => {
      if (isNaN(num)) {
        return reject(new Error('All global indexes must be numers'))
      }
    })
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this._query(query, [amount])
    }).then((rows) => {
...
DatabaseBackend.prototype.findCurrentSyncHeight = function (knownBlockHashes, startHeight, startTimestamp) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      if (startTimestamp > 0) {
        return this._query('SELECT `height` FROM `blocks` WHERE `timestamp` <= ? ORDER BY `height` DESC LIMIT 1', [startTimestamp])
      }
    }).then((rows) => {
      if (rows && Array.isArray(rows) && rows.length === 1) {
      ...
      if (startHeight > syncStart) {
        syncStart = startHeight
      }
    }).then(() => {
      return this.setCache(cacheName, syncStart, 60)
    }).then((result) => {
      return resolve(result)
    }).catch((error) => {
      return reject(error)
...
DatabaseBackend.prototype.getWalletSyncDataByHeight = function (scanHeight, blockCount) {
  ...
  return new Promise((resolve, reject) => {
    ...
    ].join(' '), [scanHeight, scanHeight + blockCount]).then((blocks) => {
      const promises = []
...
DatabaseBackend.prototype.getWalletSyncData = function (knownBlockHashes, blockCount) {
  blockCount = checkBlockCount(blockCount)
  ...
  return new Promise((resolve, reject) => {
    ...
    this.findCurrentSyncHeight(knownBlockHashes).then((syncHeight) => {
      return this.getWalletSyncDataByHeight(syncHeight, blockCount)
    }).then((results) => {
      return resolve(results)
    }).catch((error) => {
      return reject(error)
...
DatabaseBackend.prototype.legacyGetWalletSyncDataPreflight = function (startHeight, startTimestamp, blockHashCheckpoints, skipCoinbaseTransactions) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.getLastBlockHeader().then((block) => {
      ...
      return this.findCurrentSyncHeight(blockHashCheckpoints, startHeight, startTimestamp)
    }).then((syncHeight) => {
      var resolvableBlocks = result.topBlock.height - syncHeight + 1
      ...
      return this._query(blockQuery.join(' '), [syncHeight, result.topBlock.height, result.blockCount])
    }).then((blocks) => {
      blocks.forEach((block) => {
        result.blockHashes.push(block.hash)
      })
    }).then(() => {
      return resolve(result)
    }).catch((error) => {
...
DatabaseBackend.prototype.legacyGetWalletSyncDataLite = function (startHeight, blockCount) {
  blockCount = checkBlockCount(blockCount)
  ...
  return new Promise((resolve, reject) => {
    ...
    ].join(' '), [startHeight, blockCount]).then((rows) => {
      const promises = []
...
DatabaseBackend.prototype.legacyGetWalletSyncData = function (startHeight, startTimestamp, blockHashCheckpoints, blockCount, skipCoinbaseTransactions) {
  blockCount = checkBlockCount(blockCount)
  skipCoinbaseTransactions = skipCoinbaseTransactions || false
  ...
  return new Promise((resolve, reject) => {
    ...
    this.getLastBlockHeader().then((block) => {
      ...
      blockQuery.push('ORDER BY `height` ASC LIMIT ?')
      return this._query(blockQuery.join(' '), [topHeight, topBlock.height, blockCount])
    }).then((blocks) => {
...
DatabaseBackend.prototype.buildWalletDataBlock = function (block) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      ...
      return this.getTransactionsByBlock(block.blockHash)
    }).then((txns) => {
      txns.forEach((txn) => {
      ...
      return this.setCache(cacheName, obj, 60 * 60 * 24) // cache for a day
    }).then((response) => {
...
DatabaseBackend.prototype.buildLegacyWalletDataBlock = function (block, skipCoinbaseTransactions) {
  return new Promise((resolve, reject) => {
    ...
    this.checkCache(cacheName).then((cached) => {
      if (cached) {
        /* check to see if we need to skip the coinbase transaction */
        if (skipCoinbaseTransactions) {
          delete cached.coinbaseTX
        }
        return resolve(cached)
      }
    }).then(() => {
      return this.getTransactionsByBlock(block.hash)
    }).then((txns) => {
      txns.forEach((txn) => {
...
DatabaseBackend.prototype.getRandomOutputsForAmounts = function (amounts, mixin) {
  const that = this
  return new Promise((resolve, reject) => {
