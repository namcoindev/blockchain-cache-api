...
class Rabbit extends EventEmitter {
  ...
  connect () {
    return new Promise((resolve, reject) => {
      RabbitMQ.connect(this.connectionString).then((connection) => {
        connection.on('disconnect', error => this.emit('error', error))

        return connection.createChannel()
      }).then((channel) => {
        channel.on('disconnect', error => this.emit('error', error))
